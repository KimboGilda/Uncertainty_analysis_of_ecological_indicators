...
class manual_digitizing(object):
    ...
    def execute(self, parameters, messages):
        ...
        for item in range(1,numb_it + 1,1):
            ...
            #Create table
            arcpy.TableToTable_conversion(out_folder + '\\' + 'FeVeToPo.shp', out_folder, 'manual_digitalization{}.csv'.format(item))
            # Make points from table
            lyr = str(arcpy.MakeXYEventLayer_management(out_folder + '\\' + 'manual_digitalization{}.csv'.format(item), 'POINT_X', 'POINT_Y', 'xy_layer',21781).getOutput(0))
...
class georeferencing(object):
    ...
    def execute(self, parameters, messages):
        ...
        for item in range(1,int(numb_it) + 1,1):
            ...
            arcpy.management.AddGeometryAttributes(out_folder + '\\' + ' .shp', 'AREA_GEODESIC')
